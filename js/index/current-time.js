...
_cal.curTime.currentDate = () => {
  ...
  date.innerText = `${today.getFullYear()}.${_cal.splitByTwoLetters(
    today.getMonth() + 1
  )}.${_cal.splitByTwoLetters(today.getDate())}`;
};
...
_cal.curTime.currentTime = () => {
  ...
  time.innerText = `${_cal.splitByTwoLetters(today.getHours())}:${_cal.splitByTwoLetters(
    today.getMinutes()
  )}`;
};
